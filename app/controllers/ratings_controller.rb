class RatingsController < ApplicationController
  before_action :set_topic, only: %i[ create show edit update destroy ]
  ...
  def create
    @rating = @post.ratings.new(no:rating_params["star"])

    respond_to do |format|
      if @rating.save
        format.html { redirect_to topic_post_path(@topic,@post), notice: "Rating was successfully created." }
      else
        format.html { render topic_post_path(@topic,@post), status: :unprocessable_entity }
      end
    end
  end
